      ...
      <section className="pt-24 pb-12">
        <div className="container mx-auto px-4 sm:px-6">
          ...
          <div className="text-center mb-6">
            <p className="text-black text-sm sm:text-base">Published by {post.author || 'Unknown Author'}</p>
            <p className="text-black text-sm sm:text-base">
              {formattedDate}, {formattedTime}
            </p>
          </div>
          <div className="prose max-w-full sm:max-w-3xl mx-auto text-black">
            {media.length > 0 && (
              <div className="mb-6">
                {media.map((mediaItem, index) => (
                  <div key={index} className="w-full" dangerouslySetInnerHTML={{ __html: mediaItem }} />
                ))}
              </div>
            )}
          ...
          <section className="mt-12">
            ...
            <form onSubmit={handleCommentSubmit} className="mb-8 max-w-full sm:max-w-3xl mx-auto">
              <div className="mb-4">
                ...
                <input
                  ...
                  className="w-full px-4 py-2 rounded-md border border-gray-300 focus:outline-none focus:ring-2 focus:ring-sky-500 text-sm sm:text-base"
                  placeholder="Enter your name (or leave blank for Anonymous)"
                />
              </div>
              <div className="mb-4">
                <label htmlFor="commentText" className="block text-gray-700 mb-2 text-sm sm:text-base">
                  Comment
                </label>
                <textarea
                  id="commentText"
              ...
              <button
                type="submit"
                className="bg-sky-500 text-white px-4 sm:px-6 py-2 rounded-full hover:bg-sky-600 transition-colors w-full sm:w-auto"
              >
                Submit Comment
              </button>
              {commentStatus && (
                <p className={`mt-4 text-sm sm:text-base ${commentStatus.includes('successfully') ? 'text-green-500' : 'text-red-500'}`}>
                  {commentStatus}
                </p>
              )}
            </form>
            ...
            {comments.length > 0 ? (
                        ...
                        >
                          {comment.replies && comment.replies.length > 0 && (
                            <div className="ml-4 sm:ml-6 space-y-2">
                              <h4 className="text-base sm:text-lg font-semibold">Replies</h4>
                              {comment.replies.map((reply) => (
                                <div
                                  key={reply.id}
                                  className="p-2 bg-gray-100 rounded-lg"
                                >
                                  <p className="text-gray-700 text-sm sm:text-base">{reply.replyText}</p>
                          ...
                          <div className="mt-4 ml-4 sm:ml-6">
                            <h4 className="text-base sm:text-lg font-semibold">Add a Reply</h4>
                            <div className="mb-2">
                              <label htmlFor={`replyUserName-${comment.id}`} className="block text-gray-700 text-sm sm:text-base">
                                Name (optional)
                              </label>
                              <input
                                type="text"
                            ...
                            <div className="mb-2">
                              <label htmlFor={`replyText-${comment.id}`} className="block text-gray-700 text-sm sm:text-base">
                                Reply
                              </label>
                              <textarea
                            ...
                            <button
                              ...
                              className="bg-sky-500 text-white px-4 py-1 rounded-full hover:bg-sky-600 transition-colors w-full sm:w-auto"
                            >
                              Submit Reply
                            </button>
                          </div>
                ...
                {!showAllComments && comments.length > 3 && (
                  <div className="text-center mt-6">
                    <button
                      ...
                      className="bg-sky-500 text-white px-4 sm:px-6 py-2 rounded-full hover:bg-sky-600 transition-colors"
                    >
              ...
              <p className="text-center text-gray-500 text-sm sm:text-base">No comments yet. Be the first to comment!</p>
            )}
          </section>

          <div className="text-center mt-8">
            <Link href="/blog" className="text-blue-500 hover:underline text-sm sm:text-base">
              ← Back to Blog
            </Link>
          </div>
        </div>
